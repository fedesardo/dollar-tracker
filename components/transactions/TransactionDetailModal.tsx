      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-3 text-sm">
          ...
          <Row
            label="Monto"
            value={
              <Amount value={toNumber(tx.amountUsd)} size="md" showPrefix />
            }
          />
          ...
          {tx.type === 'cash_out' && (
            <>
              ...
              <Row label="Comisión" value={`${toNumber(tx.feePercentage).toFixed(2)}% · USD ${formatUSD(toNumber(tx.feeUsd))}`} tone="negative" />
              <Row label="Neto recibido" value={`USD ${formatUSD(toNumber(tx.amountUsd))}`} tone="positive" />
            </>
          )}

          {tx.beneficiary && (
            <Row label="Para" value={tx.beneficiary === 'flor' ? 'Flor' : tx.beneficiary === 'fede' ? 'Fede' : 'Ambos'} />
          )}
        ...
        {!confirmDel ? (
          ...
          <div className="rounded-xl border border-accent-red/20 bg-accent-red/5 p-3 space-y-2">
            <p className="text-sm text-text-primary">
              ¿Borrás esta transacción? <span className="text-text-muted">No hay vuelta atrás.</span>
            </p>
